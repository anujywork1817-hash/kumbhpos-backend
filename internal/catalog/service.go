...
// boolPtr converts bool to *bool — Cloudinary SDK needs pointers
...
func CreateItem(req CreateItemRequest) (Item, error) {
	...
	err := db.DB.QueryRowx(
		`INSERT INTO items (category_id, name, name_hi, price, tax_rate)
		 VALUES ($1, $2, $3, $4, $5)
		 RETURNING id, category_id, name, name_hi, price, tax_rate, is_active, image_url`,
...
func ListItems() ([]Item, error) {
	...
	err := db.DB.Select(&items,
		`SELECT id, category_id, name, name_hi, price, tax_rate, is_active, image_url
...
func GetShopCatalog(shopID string) ([]Item, error) {
	var items []Item
	err := db.DB.Select(&items,
		`SELECT i.id, i.category_id, i.name, i.name_hi, i.price, i.tax_rate, i.is_active, i.image_url
		 FROM items i
...
func UploadItemImage(itemID string, file multipart.File, header *multipart.FileHeader) (Item, error) {
	...
	result, err := cld.Upload.Upload(ctx, buf, uploader.UploadParams{
		Folder:         "kumbhpos/items",
		PublicID:       "item_" + itemID,
		Overwrite:      boolPtr(true),
		UniqueFilename: boolPtr(false),
	})
	if err != nil {
		return Item{}, fmt.Errorf("cloudinary upload failed: %w", err)
	}

	return saveImageURL(itemID, result.SecureURL)
}

func SaveItemImageURL(itemID, imageURL string) (Item, error) {
	return saveImageURL(itemID, imageURL)
}

func DeleteItemImage(itemID string) (Item, error) {
	cld, err := getCloudinary()
	if err != nil {
		return Item{}, err
	}
	...
	_, _ = cld.Upload.Destroy(ctx, uploader.DestroyParams{
		PublicID: "kumbhpos/items/item_" + itemID,
	})

	return saveImageURL(itemID, "")
}

func saveImageURL(itemID, url string) (Item, error) {
	var item Item
	err := db.DB.QueryRowx(
	...
	if err != nil {
		return Item{}, fmt.Errorf("db update failed: %w", err)
	}
	return item, nil
}
