...
import (
	...
	syncpkg "github.com/beellz/kumbhpos/internal/sync"
...
func main() {
	...
	// Item image routes
	api.POST("/items/:item_id/image", catalog.UploadImageHandler)
	api.POST("/items/:item_id/image-url", catalog.UploadImageURLHandler)
	api.DELETE("/items/:item_id/image", catalog.DeleteImageHandler)
	...
	api.POST("/sync/push", syncpkg.PushHandler)
	api.GET("/sync/pull", syncpkg.PullHandler)
	api.POST("/sync/heartbeat", syncpkg.HeartbeatHandler)
	...
	log.Printf("KumbhPOS running on :%s\n", port)
	r.Run(":" + port)
}
